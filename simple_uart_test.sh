echo "================================================================"
echo "簡單 UART TX 測試 (純 Verilog)"
echo "================================================================"

# 創建測試目錄
rm -rf uart_test_simple
mkdir uart_test_simple
cd uart_test_simple

# 1. 複製 UART TX 模塊
cp ../src/uart_tx.v .

# 2. 創建純 Verilog 的 testbench
cat > tb_uart_simple.v << 'TBEOF'
`timescale 1ns/1ps
...
echo ""
echo "編譯 UART TX 測試..."
iverilog -o uart_simple.vvp tb_uart_simple.v uart_tx.v 2>compile.log

if [ $? -eq 0 ]; then
    echo "✅ 編譯成功"
    echo ""
    echo "運行仿真..."
    vvp uart_simple.vvp 2>&1 | tee sim.log
    ...
    # 檢查波形文件
    if [ -f "uart_simple.vcd" ]; then
        echo ""
        echo "✅ 波形文件生成: uart_simple.vcd"
        echo "   大小: $(wc -c < uart_simple.vcd) 字節"
        echo ""
        echo "使用以下命令查看波形:"
        echo "  gtkwave uart_simple.vcd"
    fi
else
    echo "❌ 編譯失敗"
    cat compile.log
